...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given MFMCoin address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The MFMCoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\",        (string) The MFMCoin address validated\n"
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "to the wallet before upgrading to v0.17. To use the deprecated functionality, start mfmcoind with -deprecatedrpc=createmultisig\n"
            "\nArguments:\n"
    ...
    for (unsigned int i = 0; i < keys.size(); ++i) {
        if (IsHex(keys[i].get_str()) && (keys[i].get_str().length() == 66 || keys[i].get_str().length() == 130)) {
            ...
            " To use the deprecated functionality, start mfmcoind with -deprecatedrpc=createmultisig", keys[i].get_str()));
        }
    }
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The MFMCoin address to use for the signature.\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "mfmcoin-cli and the GUI. There is no server-side difference."
        );
...
static UniValue getinfo_deprecated(const JSONRPCRequest& request)
{
    throw JSONRPCError(RPC_METHOD_NOT_FOUND,
        ...
        "\nmfmcoin-cli has the option -getinfo to collect and format these in the old format."
    );
}
