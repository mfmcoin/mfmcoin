...
UniValue importaddress(const JSONRPCRequest& request)
{
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid mFm coin address or script");
        }
    }
    if (fRescan)
    {
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The mfmcoin address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to mfmcoind)\n"
            "\nResult:\n"
            "{                           (json object)\n"
    ...
    // produce output
    file << strprintf("# Wallet dump created by mFm coin %s\n", CLIENT_BUILD);
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see mfmcoind log for details) and could "
