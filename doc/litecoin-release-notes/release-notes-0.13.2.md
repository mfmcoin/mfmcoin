MFMCoin Core version 0.13.2 is now available from:
...
ECDSA signatures inside MFMCoin transactions now use validation using
...
Previous versions of MFMCoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of MFMCoin Core was to raise the default minimum
relay fee.

MFMCoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
MFMCoin Core 0.13.2 also introduces new default policy limits on the length and
...
MFMCoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. MFMCoin Core will relay transactions with
...
entirely in MFMCoin Core 0.13, and it is at this time undecided whether the
...
MFMCoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
MFMCoin Core automatically creates a hidden service to listen on, without
manual configuration. MFMCoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if MFMCoin Core is listening, and
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals MFMCoin
Core to use floating fees. By default, MFMCoin Core will use floating
fees.
...
At all times, MFMCoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) mFm.
Furthermore, MFMCoin Core will never create transactions paying less than
...
Various code modernizations have been done. The MFMCoin Core code base has
...
for a new opcode in the MFMCoin scripting system that in combination with
...
MFMCoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of MFMCoin Core.
...
The two phases are distinct in the MFMCoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming MFMCoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for MFMCoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each MFMCoin block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** MFMCoin addresses (both P2PKH addresses
  ...
  believed to be provided by MFMCoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of MFMCoin's current ledger (called the UTXO set),
  ...
  note that MFMCoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  MFMCoin users to individually opt-in to almost any change in the MFMCoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and MFMCoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the MFMCoin
client to MFMCoin Core 0.13.2 to signal for segwit support. When a
...
Since MFMCoin Core 0.10.0, nodes have defaulted to only relaying and
