mFm coin Core version *0.15.0* is now available from:
...
installer (on Windows) or just copy over `/Applications/mFm coin-Qt` (on Mac)
...
mFm coin Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.8+, and Windows Vista and later. Windows XP is not supported.

mFm coin Core should also work on most other Unix-like systems but is not
...
- The `estimatesmartfee` RPC interface has been changed (See [PR 10707](https://github.com/bitcoin/bitcoin/pull/10707)):
    ...
    - If mFm coin Core has not been running for long enough and has not seen enough blocks or transactions to produce an accurate fee estimation, an error will be returned (previously a value of -1 was used to indicate an error, which could be confused for a feerate).
...
mFm coin Core now supports loading multiple, separate wallets (See [PR 8694](https://github.com/bitcoin/bitcoin/pull/8694), [PR 10849](https://github.com/bitcoin/bitcoin/pull/10849)). The wallets are completely separated, with individual balances, keys and received transactions.

Multi-wallet is enabled by using more than one `-wallet` argument when starting mFm coin, either on the command line or in the mFm coin config file.
...
mFm coin Core 0.15.0 contains the following changes to the RPC interface and `mfmcoin-cli` for multi-wallet:

* When running mFm coin Core with a single wallet, there are **no** changes to the RPC interface or `mfmcoin-cli`. All RPC calls and `mfmcoin-cli` commands continue to work as before.
* When running mFm coin Core with multi-wallet, all *node-level* RPC methods continue to work as before. HTTP RPC requests should be send to the normal `<RPC IP address>:<RPC port>/` endpoint, and `mfmcoin-cli` commands should be run as before. A *node-level* RPC method is any method which does not require access to the wallet.
...
- When using mFm coin Core in multi-wallet mode, RPC requests for wallet methods must specify
